...
function HotelRegister() {
  ...
  const cities = useSelector((state) => state.city.dataCity);
  ...
  cities.map((e) => selectCities.push(e));

  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(fetchingCities());
    const optionsPhone = listOfCountries.map((country) => country.phone);
    const phoneSet = [...new Set(optionsPhone)].sort(
  ...
  return (
    <div className=" mt-20 inset-0 bg-neutral-100 pb-20">
      ...
      >
        ...
        />
        <span className="text-red-400">
          {errors.address && <p>{errors.numberRooms}</p>}
        </span>

        <label
          className={`${josefinRegular.className} mt-2.5`}
          htmlFor="hotelEmailInput"
        >
          Business email
        </label>
        <input
        ...
        <label
          className={`${josefinRegular.className} mt-2.5`}
          htmlFor="hotelPhoneInput"
        >
          Business phone number
        </label>
        <div>
          <select
            onChange={selectLadaChange}
            className={`${josefinRegular.className} border-2 bg-slate-50 rounded-xl mr-2 my-2 pl-3 py-3 pb-3 w-1/3`}
            id="select"
          >
            {phoneCode.map((phone) => (
          ...
          <input
            className={`${josefinRegular.className} border-2 rounded-xl my-2 pl-3 py-3 pb-3 w-52`}
            type="text"
        ...
        <div className="flex gap-5 ">
          <div className="flex flex-col flex-1 ">
            ...
            <input
              className={`${josefinRegular.className} border-2 rounded-xl my-2 pl-3 py-3 pb-3`}
              type="time"
              onChange={handleChange}
              id="checkIn"
              name="checkIn"
        ...
        />
        <span className="text-red-400">
          {errors.overview && <p>{errors.overview}</p>}
        </span>
        ...
        errors.overview ? (
          <button
            className={`${josefinRegular.className} bg-[#929292] text-white mt-10 h-10 mx-auto rounded-full w-3/4`}
            disabled
          >
            Create hotel
          </button>
          ...
          <button
            type="submit"
            className={`${josefinRegular.className} bg-[#7533ac] text-white mt-10 h-10 mx-auto rounded-full w-3/4`}
          >
            Create hotel
          </button>
        )}
