  ...
  return (
    <div className="p-5 pb-24">
      <div>
        ...
        <div
          className={`  shadow-insetContainerTypeRooms border pb-5 rounded-2xl h w-ful grid grid-cols-1 gap-3`}
        >
          {rooms &&
            rooms?.map((roomType) => {
              return <ContainerTypesRooms roomType={roomType} />;
            })}
          <div className=" flex flex-wrap justify-center items-center">
            <Link
              href={`/createRoom/?id=${hotel.id}`}
              className="relative border-2 border-green-600 w-10 h-10 flex justify-center items-center rounded-full shadow-buttonAdd"
            >
