...
function RoomCreator() {
  ...
  const services = useSelector((state) => state.services.dataService);

  services.map((e) =>
    selectServices.push({ label: e.name, value: [e.id, e.name] })
  );
  ...
  const handleCreate = async (e) => {
    if (newService) {
      try {
        ...
        setForm({
          ...form,
          ServicesRoom: [...form.ServicesRoom, response.data.id],
        });
  ...
  const handleDelete = (e) => {
    const updatedServiceName = serviceName.filter(
  ...
  return (
    <div className="relativeflex items-center justify-center  bg-neutral-100 pb-20 inset-0">
      <form
        ...
        className="bg-neutral-50  mt-5 mb-32 flex flex-col p-4 shadow-md"
      >
        ...
        <input
          className={`  border-2 rounded-xl my-2 pl-3 py-3 pb-3  `}
          type="text"
          name="room"
          id="type"
        ...
        <div className="flex gap-5 ">
          <div className="flex flex-col w-5/12 ">
            <label className={` mt-2.5`} htmlFor="price">
              Price
            </label>
          ...
          <div className="flex flex-col w-5/12">
            <label className={` mt-2.5 `} htmlFor="RoomsNumber">
              Rooms
            </label>
        ...
        <label className={` mt-2.5`} htmlFor="roomDescription">
          Description
        </label>
        <input
          className={` border-2 rounded-xl my-2 h-14 pl-4`}
          type="text"
          onChange={handleChange}
          id="roomDescription"
          name="description"
        ...
        <label className={`  my-2.5`} htmlFor="addRoom">
          Services
        </label>
        ...
        <div className="grid bg-neutral-50 grid-cols-3 rounded-xl my-4 gap-4  border-2 justify-between p-2">
          {serviceName &&
            serviceName?.map((c) => (
              <span
                className={` flex items-center h-full text-center relative  text-md text-white bg-[#7533ac] rounded-xl p-2`}
              >
                {c.name}
                <button
                  type="button"
                  className="w-2 text-lg p-0 absolute -top-0.5 right-0 mr-2"
        ...
        <label className={` mt-2.5`} htmlFor="NewServices">
          New service
        </label>
        <input
          className={` border-2 rounded-xl mt-2 h-14 pl-4`}
          type="text"
        ...
        {newService ? (
          <button
            ...
            className={` bg-[#7533ac] text-white mt-5 h-10 mx-auto rounded-full w-2/4`}
          >
            Create service
          </button>
        ) : (
          <button
            type="button"
            disabled
            className={` bg-gray-500 text-white mt-5 h-10 mx-auto rounded-full w-2/4`}
          >
            Create service
          </button>
        )}
        ...
        {!form.room || !form.price || !form.description ? (
          <button
            className={` bg-[#929292]  text-white mt-10 h-10 mx-auto rounded-full w-3/4`}
            disabled
          >
            Create room
          </button>
          ...
          <button
            ...
            type="submit"
            className={` bg-[#7533ac] text-white mt-10 h-10 mx-auto rounded-full w-3/4`}
          >
            Create room
          </button>
        )}
