...
const HotSaleSlider = ({ roboto }) => {
  ...
  useEffect(() => {
    const fetchParis = async () => {
      try {
        const response = await axios.get(
          "/hotel/findhotel?name=Hotel du Collectionneur"
        );
    ...
    const fetchBali = async () => {
      try {
        const response = await axios.get(
          "/hotel/findhotel?name=The Kayon Valley Resort"
        );
  ...
  return (
    <div className=" grid grid-cols-2 mt-5 gap-5">
      <div className="flex flex-col justify-between">
        {paris && paris ? (
          <button
            className="relative h-56 shadow-img rounded-3xl"
            onClick={() => {
          ...
          >
            ...
            />
            <div
              className={` text-white absolute bottom-3 w-full p-3 pb-0 h-20 leading-4`}
            >
              <h2 className={`${roboto.className} tracking-wide  text-sm `}>
                {paris?.name}
              </h2>
              <div className="flex justify-between items-center">
                <h2 className={`${roboto.className} tracking-wider text-sm`}>
                  Paris
                </h2>
