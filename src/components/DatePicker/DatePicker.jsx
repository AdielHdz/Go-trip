...
const DatePick = ({ open, setInputIsDisabled, toggleOpen }) => {
  ...
  const [datesStrings, setDatesString] = useState({
    arrival: "",
    exit: "",
  });
  ...
  const disabledDate = (current) => {
    ...
    return (
      current.isBefore(currentDatePlusOne) ||
      current.isAfter(threeMonthsFromNow)
    );
  };

  const disabledDateExit = (current) => {
    if (!current) {
      return false;
    }
    ...
    const desiredMinimumDate = dayjs(updatedDateString, "DD-MM-YYYY").startOf(
      "day"
    );
    const threeMonthsFromNow = dayjs().add(5, "month").startOf("day");
    return (
      current.isBefore(desiredMinimumDate) ||
      current.isAfter(threeMonthsFromNow)
    );
  };

  const handleDateChange = (values, dateString) => {
    setDates(dateString);
  };

  const handleDateString1 = (values, dateString) => {
    setDatesString({ ...datesStrings, arrival: dateString });
  };

  const handleDateString2 = (values, dateString) => {
